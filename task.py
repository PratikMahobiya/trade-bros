...
def FnO_BreakOut_1(auto_trigger=True):
    ...
    try:
        ...
        index_list = [ 'NIFTY', 'BANKNIFTY', 'MIDCPNIFTY', 'FINNIFTY', 'NIFTYNXT50', 'SENSEX', 'BANKEX']
...
def CheckFnOLtp():
    ...
    try:
        ...
        for sym in symbol_obj_list:
            if sym.symbol.name in ['NIFTY', 'BANKNIFTY', 'MIDCPNIFTY', 'FINNIFTY', 'NIFTYNXT50', 'SENSEX', 'BANKEX']:
                yfsymb = {
                    'NIFTY': '^NSEI',
                    'BANKNIFTY': '^NSEBANK',
                    'MIDCPNIFTY' : 'NIFTY_MID_SELECT.NS',
        ...
        for ticker in tickers:
            try:
                # ltp = tickers[ticker].info.get('regularMarketPrice')
                name_ = next((key for key, val in yfsymb.items() if val == ticker), None)

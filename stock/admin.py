...
class FnOStatusAdmin(ExtraButtonsMixin, admin.ModelAdmin):
    actions = ["exit_positions"]
    list_display = ('entry_time', 'name_', 'current', 'max_p', 'max_l_s', 'chart_price', 'ltp', 'price', 'fixed_target', 'stoploss', 'manual_updated', 'trailing_sl', 'target', 'highest_price', 'entry_open_value', 'entry_prev_close_value', 'lot', 'tr_hit', 'capital_save', 'mode', 'product')
    ...
    @admin.action(description="Exit the selected positions")
    def exit_positions(self, request, queryset):
        try:
            now = datetime.now(tz=ZoneInfo("Asia/Kolkata"))
            check_holiday = Holiday.objects.filter(date=now.date(), is_active=True)
            if check_holiday:
                raise Exception(f"Today is Holiday : {check_holiday[0].holiday}")
            if now.time() < time(9, 15, 00):
                raise Exception(f"Try to exit in Market hours")
            elif now.time() > time(15, 30, 00):
                raise Exception(f"Try to exit in Market hours")
            configuration_obj = Configuration.objects.filter(product='future')[0]
            for stock_obj in queryset:
                try:
                    data = {
                            'exit_type': 'M-Exit',
                    ...
                    self.message_user(request, f'Successful Exit: {stock_obj.symbol.symbol}', level=messages.SUCCESS)
                except Exception as e:
                    self.message_user(request, f'Failed Exit: {stock_obj.symbol.symbol} : Error : {e}', level=messages.ERROR)
        except Exception as e:
            self.message_user(request, f'Failed Exit : Error : {e}', level=messages.ERROR)
    ...
    def STATUS(self, request):
        ...
        future_till_now = round(sum(Transaction.objects.filter(product='future', indicate='EXIT', type__in=['TARGET', 'STOPLOSS', 'TR-SL', 'PIVOT', 'SQ-OFF'], is_active=True).values_list('profit', flat=True)), 2)
        future_today_cs_exit = Transaction.objects.filter(date__date=now.date(), product='future', indicate='EXIT', type__in=['CS-TARGET'], is_active=True).order_by('date').count()
        future_today_exit = Transaction.objects.filter(date__date=now.date(), product='future', indicate='EXIT', type__in=['TARGET', 'STOPLOSS', 'TR-SL', 'PIVOT', 'SQ-OFF'], is_active=True).order_by('date').count()
        ...
        future_today = round(sum(Transaction.objects.filter(date__date=now.date(), product='future', indicate='EXIT', type__in=['TARGET', 'STOPLOSS', 'TR-SL', 'PIVOT', 'SQ-OFF'], is_active=True).values_list('profit', flat=True)), 2)
...
#     # actions = None
#     list_display = ('entry_time', 'name_', 'current', 'max_p', 'max_l_s', 'ltp', 'fixed_target', 'price', 'stoploss', 'trailing_sl', 'target', 'highest_price', 'indics', 'lot', 'capital_save', 'tr_hit', 'mode', 'product')
...
class FnOTransactionAdmin(ExportActionMixin, admin.ModelAdmin):
    date_hierarchy = 'date'
    list_display = ('symbol', 'date', 'indicate', 'type', 'p_l', 'max_p', 'max_l_s', 'chart_price', 'price', 'fixed_target', 'stoploss', 'target', 'top_p', 'lot', 'name', 'token', 'exchange', 'mode')
    search_fields = ['symbol', ]
    list_filter = ('indicate', 'date', 'mode', 'type', 'name')
    list_per_page = 20
...
class StockConfigAdmin(ExtraButtonsMixin, admin.ModelAdmin):
    list_display = ('created_at', 'mode', 'symbol__name', 'ltp', 'manual_updated', 'capital_save', 'tr_hit', 'trailing_sl', 'max', 'max_l', 'price', 'chart_price', 'target', 'stoploss', 'fixed_target', 'highest_price', 'entry_open_value', 'entry_prev_close_value', 'symbol__symbol', 'lot', 'is_active')
    search_fields = ['symbol', ]
...
class TransactionAdmin(ExportActionMixin, admin.ModelAdmin):
    date_hierarchy = 'date'
    list_display = ('product', 'symbol', 'date', 'indicate', 'type', 'p_l', 'max_p', 'max_l_s', 'chart_price', 'price', 'fixed_target', 'stoploss', 'target', 'top_p', 'lot', 'name', 'token', 'exchange', 'mode')
    search_fields = ['symbol', ]
    list_filter = ('product', 'indicate', 'date', 'mode', 'type', 'name')
    list_per_page = 20

...
def TrailingStopLossExit(data, ltp, open_position, correlation_id, socket_mode, sws):
    ...
    if (not data['stock_obj'].tr_hit and data['stock_obj'].mode == 'CE' and ltp <= price_value) or (not data['stock_obj'].tr_hit and data['stock_obj'].mode == 'PE' and ltp >= price_value) or data['percent'] < -data['configuration_obj'].stoploss or (data['stock_obj'].tr_hit and ltp <= price_value):
        # Exit Order.
